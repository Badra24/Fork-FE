import {
 ...
 import { axiosInstance } from "../../../../../lib/api";
  ...
  export default function UpdateTabel(props) {
    const { idUpStock, upCapital ,upProfit,upSold_qty ,upStock,upSelling,upStockunit } = props;
    ...
    const formik = useFormik({
      ...
      onSubmit: async () => {
        try {
          await axiosInstance.patch(`/product_stock/update/` + idUpStock,formik.values).then(() => {
            dispatch({
              type: "RENDER_DATA",
              payload:{
            ...
            <ModalBody pb={6}>
              <Box display='flex' flexWrap='wrap' justifyContent='space-evenly'>
                ...
                <Box mt='10px'>
                  ...
                  <FormControl>
                    <FormLabel>primary stock {formik.values.primary_stock}</FormLabel>
                    
                    <Textarea placeholder='primary_stock' maxLength='2000' w='400px' h='150px'
                      onChange={(e) => {
                  ...
                  <FormControl>
                    <FormLabel>primary unit {formik.values.primary_unit}</FormLabel>
                    
                    <Textarea placeholder='primary_unit' maxLength='2000' w='400px' h='150px'
                      onChange={(e) => {
